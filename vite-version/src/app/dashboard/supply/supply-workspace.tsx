...
export default function SupplyWorkspace() {
  ...
  return <BaseLayout><SupplyContext.Provider value={{ data: supplyPreview, context, analysis, update }}>
    <main className="supply-workspace">
      ...
      <div className="si-preview-notice"><span className="si-tag">Demonstration world</span><p>Illustrative supply, costs and obligations. No live inventory connected.</p><details><summary>Data basis</summary><div><p>Snapshot: 13 Sep 2026. The processor node is Evergreen Wood Industries Ltd (Mpigi District, Uganda) -- a real, independently-verified plywood/veneer manufacturer (NEMA registration + NBD Trade Data customs export records). "Zurkt Uganda" is carried only as a documented trade-partner alias (a Chinese buyer named in the same customs records); no independent source confirms Zurkt Group owns or operates this factory. The nearby forests shown ARE real, canonical Uganda CFR AOIs with real distances and real EO observation coverage from the operational database; their stand volume, grade mix and supply/cost figures are a Monte Carlo model over explicitly ASSUMED priors (see the model summary below), not field measurements. No canonical administrative API is called.</p><p>G1 uses a 30 cm legacy tree-DBH threshold; G2/G3 assume 20/15 cm from the roundwood scenario library. All use 2.7 m minimum length. These are constructed scenario thresholds, not Evergreen's actual veneer-log buying spec, which is not published or on file. No volume-to-tonne conversion is applied to the preview.</p><p>“Verified” in the preview pipeline is only a simulated commercial stage. All displayed supply remains unverified synthetic evidence.</p></div></details></div>
