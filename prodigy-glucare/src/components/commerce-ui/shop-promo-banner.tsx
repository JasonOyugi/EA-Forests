...
export function ShopPromoBanner({
  ...
  title,
  description,
  gradientClass = "from-emerald-100 via-emerald-200 to-aquamarine-100",
  accentClass = "text-emerald-700",
  scrollTargetId = "new-arrivals-section",
  ...
  return (
    <BentoTilt>
      <div
        className={`relative w-full overflow-hidden rounded-lg bg-gradient-to-r ${gradientClass} p-8 text-slate-900 shadow-xl`}
        onClick={() => {
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-between space-y-6 pr-8 md:flex-row md:space-y-0 md:pr-10">
          ...
          >
            ...
            <p className="mt-3 text-sm text-slate-700 md:text-base">{description}</p>
          </div>
        </div>
      </div>
    </BentoTilt>
